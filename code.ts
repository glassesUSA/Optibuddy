...
function organiseAssets() {
  exportElements = []
  let assetsPage = figma.currentPage.findChildren(
    (n) => n.getPluginData('key') == 'assets',
  )
  if (assetsPage) {
    assetsPage.forEach((r) => r.remove())
  }
  let frames = figma.currentPage.findAll(
    (t) =>
  ...
  figma.currentPage.selection.forEach((e) => {
    if (
      ...
      if (e.name.includes('Components')) {
        ...
        e.findAll((r) => r.name.includes('Components')).forEach((r) =>
          elements.push(r),
        )
      }

      if (elements.length)
        elements.forEach(async (r) => {
          ...
          if (path[2].includes('CTA') || path[1] == 'Buttons') {
            ...
            if (path[2].includes('CTA')) {
              compSet = figma.importComponentSetByKeyAsync(
                'e7cd7d5df859f90a000c66e0cda84c6abe535bb2',
              )
            } else {
              compSet = figma.importComponentSetByKeyAsync(
                'c918ee09c4bce3804100a8dc51a6aa3ff9dff40a',
              )
            }
            let c = await compSet
            let colour = path[3]
            ...
            if (colour == 'Multicolored') colour = 'Black'

            let filtered = c.children.filter((t) => {
              let cName = t.name.split(',')
              if (path[2] == 'CTA') {
                if (
                  cName[0].replace(/.+?=/g, '').toLowerCase() ==
...
function getElements(e, action) {
  ...
  if (--elementRunning == 0) {
    if (action == 'check') {
      if (errorNodes.length > 0) figma.currentPage.selection = errorNodes

      if (errorNodes.length > 0) {
        let errorMessage = ''
        for (let [key, value] of Object.entries(styleErrors)) {
          let x = `<div class="styleTitle" onclick="selectNode('${key}')">${
            figma.getNodeById(key).name
          }</div>`
          // @ts-ignore
...
function convertFrames() {
  ...
  let e = figma.currentPage.selection.forEach((x) => {
    ...
    x.findAll((r) => r.type == 'FRAME' && r.parent.type != 'INSTANCE').forEach(
      (e) => {
    ...
    if (
      ...
      if (x.type == 'GROUP' && x.children.length == 1) {
        try {
          ...
          if ('expanded' in x.parent) x.parent.expanded = true
      ...
      x.findAll(
        (r) => r.type == 'GROUP' && r.parent.type != 'INSTANCE',
      ).forEach((e) => {
        if (e.type == 'GROUP' && e.children.length == 1) {
          try {
            if ('expanded' in e.parent) e.parent.expanded = true
...
function checkText(el, width) {
  let id = el.id
  if (el.fontName === 'Symbol(figma.mixed') return
...
function convertText(el, width) {
  ...
  if (el.fontName['family'] != 'Roboto') {
    if (width < 768 && style === 'Medium') style = 'Bold'
    fontCount++
...
function checkColour(e) {
  ...
  if (e.fills.length == 1) {
    ...
    for (let x in defaultColours) {
      ...
      if (
        convertedFill.r == currentColour.r &&
  ...
  if (e.strokes.length == 1) {
    ...
    for (let x in defaultColours) {
      ...
      if (
        ...
        if (e.fillStyleId && e.fills[0].opacity == 1) return
        figma.importStyleByKeyAsync(currentColour.id).then((s) => {
...
async function addFrame(size) {
  figma.ui.postMessage({ type: 'loading', data: 'Inserting grid...' })
  let s, width, height, name

  switch (size) {
    case 1440:
      s = await figma.importStyleByKeyAsync(
        '138a91c709a4eebec70ea185f185647d49577de7',
      )
      width = 1440
      height = 900
      name = 'Desktop - 1440px'
      break
    case 1280:
      s = figma.importStyleByKeyAsync(
        '1b06e2b9dc595a2693a141d2b4f2743b014c14f0',
      )
      width = 1280
      height = 720
      name = 'Desktop - 1280px'
    ...
    case 1024:
      s = figma.importStyleByKeyAsync(
        '03e795f5a654dab76a3d0d028685b04b63e1147d',
      )
      width = 1024
      height = 1399
      name = 'Desktop - 1024px'
    ...
    case 375:
      s = figma.importStyleByKeyAsync(
        '2005abbc6d1a00d728552a30e3caaec3e4cb3ea4',
      )
      width = 375
      height = 667
      name = 'Mobile - 375px'
    ...
    if ('children' in u) {
      u.findAll((r) => r.type == 'INSTANCE').forEach((u) => {
        if ('mainComponent' in u) {
          ...
          if (u.mainComponent.remote) return
          u.detachInstance()
        }
      })
    }
...
function connectColour() {
  figma.currentPage.selection.forEach((e) => {
    ...
    if ('children' in e || 'fills' in e || 'strokes' in e) {
      ...
      nodes.forEach((r) => {
        if (r.node.fillStyleId != '' && r.node.strokeStyleId != '') return
        let i = r.color[0]
        if (i.opacity != 1 || !i.visible) return
        let f = i.color
        ...
        for (let x in defaultColours) {
          ...
          if (
            ...
            figma.importStyleByKeyAsync(currentColour.id).then((s) => {
              if (r.type == 'fill') r.node.fillStyleId = s.id
              else r.node.strokeStyleId = s.id
            })
          }
        }
      })
...
figma.on('run', ({ command, parameters }: RunEvent) => {
  switch (command) {
    ...
    case 'cleanOut':
      cleanPage().then(() => {
        figma.closePlugin(
          'Moved all elements outside frames into the Archive page',
        )
      })
      break
  }
...
if (figma.command === 'getColour') getColours()
